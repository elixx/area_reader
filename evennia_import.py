        ...
        for r in self.area_file.area.resets:
            if r.command == 'M':
                try:
                    ...
                    log_err(
                        "! enumerateMobLocations(): " + str(e) + "\n\tmob_vnum=v" + str(r.arg1) + " room_vnum=v" + str(r.arg3))
        ...
        for r in self.area_file.area.resets:
            ...
            if r.command == 'M':
                ...
                try:
                    ...
                    log_err(
                        "! enumerateObjectLocations(O): " + str(e) + "\n\tobject_vnum=v" + str(r.arg1) + " room_vnum=v" + str(r.arg3))
            elif r.command == 'E':
                try:
                    object = r.arg1
                    wear_loc = r.arg3
                    if last_mob_vnum is not None:
                        ...
                        log_info(f"enumerateObjectLocations(E): Object v{object} equipped on mob v{last_mob_vnum} at wear_loc {wear_loc}")
                    else:
                        log_err(f"! enumerateObjectLocations(E): Object v{object} has E command but no previous M command")
                except Exception as e:
                    log_err(
                        "! enumerateObjectLocations(E): " + str(e) + "\n\tobject_vnum=v" + str(r.arg1) + " wear_location=" + str(r.arg3))
            elif r.command == 'G':
                try:
                    object = r.arg1
                    if last_mob_vnum is not None:
                        ...
                        self.mob_equipment[object] = last_mob_vnum
                        log_info(f"enumerateObjectLocations(G): Object v{object} given to mob v{last_mob_vnum} inventory")
                    else:
                        log_err(f"! enumerateObjectLocations(G): Object v{object} has G command but no previous M command")
                except Exception as e:
                    log_err(
                ...
                try:
                    ...
                    log_err(
                        "! enumerateObjectLocations(P): " + str(e) + "\n\tobject_vnum=v" + str(r.arg1) + " container_vnum=v" + str(r.arg3))
    ...
    def spawnRooms(self):
        if self.rooms_created:
            ...
            for vnum in sorted(self.rooms):
                ...
                if (firstRoom):
                    log_warn("* Entry to " + room['area'] + ' - #' + str(newroom.id) + " (v" + str(vnum) + ") = " + room['name'])
        ...
        if self.exits_created:
            ...
            for vnum in sorted(self.rooms):
                ...
                for exit, exitData in room['exits'].items():
                    ...
                    if exitData['dest'] not in self.room_translate.keys():
                        ...
                        if exitData['dest'] == -1:
                            ...
                            log_info(f"Creating one-way exit '{exitDir}' in {room['area']} room v{vnum} (nowhere destination)")
                            ...
                            log_warn(f"Creating one-way exit '{exitDir}' in {room['area']} room v{vnum} -> cross-area destination v{exitData['dest']}")
                            create_object(typeclass=EXIT_TYPECLASS,
                                          key=exitDir, location=loc, destination=None, aliases=[DIRALIAS[exitDir]],
                                          tags=[(room['area'], 'area'),
                                                (room['area'], 'exit'),
                    ...
                    try:
                        ...
                        if dest is None:
                            log_err('! no destination object: ' + room['area'] + ": Exit " + exitDir + " in EVid " + str(
                                evid) + " (v" + str(vnum) + ") skipped, v" + str(exitData['dest']) + " not found.")
                            continue
                    except Exception as e:
                        log_err(f"! destination lookup failed: {room['area']}: Exit {exitDir} in EVid {evid} (v{vnum}) -> v{exitData['dest']}: {str(e)}")
                        continue
                    create_object(typeclass=EXIT_TYPECLASS,
                ...
                if count == 0:
                    ...
                    log_warn(f"Blackhole room detected - vnum v{vnum}, EvId #{self.room_translate[vnum]} in {room['area']} - '{room['name']}'")
            ...
            log_info(f"Exit creation summary for {self.areaname}: {successful_exits}/{total_exits} successful, {nowhere_exits} nowhere, {cross_area_exits} cross-area, {blackhole_rooms} blackhole rooms")
        ...
        if self.objects_created:
            ...
            for vnum in sorted(self.objects):
                ...
                if vnum in self.mob_equipment:
                    ...
                    if mob_vnum in self.mob_translate:
                        ...
                        try:
                            ...
                            log_info(f"spawnObjects(): Giving object v{vnum} ({ob['name']}) to mob v{mob_vnum} evid:{mob_evid}")
                        except Exception as e:
                ...
                # Check if this object should be placed in a container
                    ...
                    if container_vnum in self.object_translate:
                        ...
                        try:
                            ...
                            log_info(f"spawnObjects(): Placing object v{vnum} ({ob['name']}) inside container v{container_vnum} evid:{container_evid}")
                        except Exception as e:
                            log_err(f"! spawnObjects(): Failed to find container v{container_vnum} for object v{vnum}: {str(e)}")
                            continue
                    else:
                        log_err(f"! spawnObjects(): Container v{container_vnum} not found in object_translate for object v{vnum}")
                        continue
                ...
                # Check if this object should be placed in a room
                    ...
                    log_err("! vnum v%s not found in object_location, mob_equipment, or container_objects for %s" % (vnum, ob['name']))
                    continue
